...
fn main() {
    ...
    let client_base = process
        ...
        println!("Client Address: {:x}", client_base);
    // Get engine.dll
    let engine_base = process
        ...
        .info
        .baseAddress as u32;
        println!("Engine Address: {:x}", engine_base);
    ...
    loop {
       ...
       //println!("localplayer Address: {:x}", local_player);
        let local_team: u32 = process.read(&c_ctx, (local_player + offsets::m_iTeamNum) as u64);
 
        if local_team != 0 {
            //println!("local_team  Value: {}", local_team);
        }
        ...
        if crosshair_entity_health != 0 {
           // println!("crosshair_entity team Value: {}", crosshair_entity_team);
        }
